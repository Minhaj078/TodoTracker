...
//     document.getElementById('habitTime').value = task.timeOfDay || '';
...
//     <tr>
//       <th>Habit</th>
//       ${weekDates.map(date => `<th>${new Date(date).toDateString().slice(0, 3)}</th>`).join('')}
//       <th>🔥 Streak</th>
//     </tr>`;
...
//         return `<td><input type="checkbox" onchange="toggleHabitComplete('${habit.id}', '${date}')" ${checked}></td>`;
//       }).join('')}
...
// function toggleHabitComplete(id, date) {
//   const task = tasks.find(t => t.id === id);
